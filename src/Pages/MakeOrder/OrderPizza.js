...
function OrderPizza(props) {
	...
	function handleChoose(name, sliceExtras) {
		...
		const checkChoose = orderExtras.some((orderExtra) => orderExtra.name === name + " " + chooseBox.extra && orderExtra.slices[0] === sliceExtras[0]);
	...
	function extraOnPizza(pizza_slice) {
		...
		for (let i = 0; i < 12; i++) {
			...
			if (window.innerWidth >= 768 && window.innerWidth < 992) {
				...
				while (
					(width <= 60 && height >= 170) ||
					(width > 60 && width <= 80 && height >= 160) ||
					(width > 80 && width <= 100 && height >= 150) ||
					(width > 100 && width <= 120 && height >= 140) ||
				...
				while (
					(width <= 20 && height >= 80) ||
					(width > 20 && width <= 40 && height >= 70) ||
					(width > 40 && width <= 60 && height >= 60) ||
					(width > 60 && width <= 70 && height >= 40) ||
					(width > 70 && width <= 80 && height >= 30)
				) {
					height = Math.floor(Math.random() * 90).toString();
				}
			}
		...
		return arrayExtra.map((extra, index) => <img style={arrayPosition[index]} src={"../images/extras/" + extra + ".png"} />);
	}
		...
		<div className={props.className === "hidden" ? "hidden" : "opgrid"}>
										...
										{(provided, snapshot) => {
											return (
												<div
													...
													className={snapshot.isDraggingOver ? pizza_slice + " " + "lighting" : pizza_slice}></div>
											);
										}}
									</Droppable>
									<div className={pizza_slice}>{extraOnPizza(pizza_slice)}</div>
								</div>
							);
						})}
					</div>
			...
			<div className="opmanageextras">
				...
				{orderExtras.map((orderExtra) => {
					return (
						<div>
							<text>{orderExtra.name}</text>
							<FontAwesomeIcon onClick={() => removeOrderExtra(orderExtra)} icon={faTimes} />
						</div>
					);
				})}
			</div>
